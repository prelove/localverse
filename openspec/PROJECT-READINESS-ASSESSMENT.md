...
##### Task 003: Finder 插件 (12h) - 🔵 开发中
**状态**: 核心结构与服务接入中，功能逐步补齐
...
##### Task 004: Wiki 插件 (16h) - 🔵 开发中
**状态**: 基础功能开发中（模块/列/卡片与编辑器补齐中）
...
1. **Task 003: Finder 插件** (12h) 🔵 **进行中**
   - **原因**: 搜索服务已就绪，需补齐索引/预览链路
   - **产出**: 文件搜索界面、文件索引、预览
   - **验收**: 能搜索本地文件并预览

2. **Task 004: Wiki 插件** (16h) 🟡 **MVP 核心功能**
   - **依赖**: 前端核心 + 数据库服务
...
**组 B: 回归验证**
- 人员 A: Task 002 (搜索服务回归)

### 3. 风险管理

**高风险项**:
1. 插件系统复杂度可能超预期 (已完成，风险已降低)
2. 前后端集成可能遇到跨域/协议问题
...
**🟡 部分可运行**: 插件系统与前端核心已完成，插件功能仍在补齐

### 距离可运行的差距

**最小可运行版本（单机 Wiki）**: 
- **剩余任务**: 2 个（Task 004 Wiki, 搜索/数据库回归）
- **预估工时**: 24 小时（~3个工作日）
- **关键阻塞**: Task 004 Wiki 插件

**MVP 版本（Wiki + Finder）**:
- **剩余任务**: 3 个（Task 003 Finder, Task 004 Wiki, 搜索回归）
- **预估工时**: 36 小时（~5个工作日）
- **关键依赖**: Phase 0 全部完成
...
**立即行动**:
1. 🔍 **推进 Task 003（Finder 插件）** - 补齐索引与预览
2. 📚 **启动 Task 004（Wiki 插件）** - 交付 MVP
3. ✅ **完成 Task 002 回归验证** - 稳定搜索链路
