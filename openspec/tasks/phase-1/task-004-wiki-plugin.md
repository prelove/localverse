...
- 2026-02-06: 强化跨模块链接解析与反向链接上下文、优化编辑提醒与标签交互、补充搜索摘要与定位体验。
- 2026-02-06: 关闭详情时补充未保存提示，确保编辑流程可安全回退。
- 2026-02-06: 链接解析增加上下文提示并按模块/更新时间排序，强化重名链接跳转一致性。

- 2026-02-17: 新增 Wiki 服务层单元测试（LinkParser/VersionManager），验证链接解析、反向链接与版本比较/恢复路径。
- 2026-02-28: 新增 WikiService CRUD 单元测试（10项，含模块/列/卡片/链接/版本历史），为 wiki/index.js 添加拖拽支持（dragstart/dragover/drop），更新所有验收标准为已完成。
